...
import LEAStereo.cell_level_search_2d as cell_level_search
...
class DisparityRegression(nn.Module):
    ...
    def forward(self, x):
        ...
        disp = torch.reshape(torch.arange(0, self.maxdisp, dtype=torch.float32), [1, self.maxdisp, 1, 1]).to(self.device)
...
class Disp(nn.Module):
    def __init__(self, maxdisp, device):
        ...
        self.device = device
        self.softmax = nn.Softmin(dim=1)
        self.disparity = DisparityRegression(maxdisp, device)
